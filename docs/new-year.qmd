...
new_joiners_text <- sprintf("The most recent %s <b>%s</b> with %s.", is_are, tz_ids_combined, scales::number(
      new_joiners_pop, 
      accuracy = ifelse(new_joiners_pop > 1e9, 0.01, 1), 
...
## And which share of the world population is [in the New Year]{style="color:#52FFEE"}?
...
donut_label <- 
  sprintf(
    ...
    scales::number(
      ...
      accuracy = ifelse(
        pop_shares_new_year$total_pop[pop_shares_new_year$is_new_year] > 1e9,
        0.01,
        1),
      scale_cut = scales::cut_short_scale(),
      prefix = "approx. ", suffix = " people")
